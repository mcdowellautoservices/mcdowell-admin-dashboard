...
export default function App() {
  ...
  function hasCustomerGps(job) {
    return (
      job.customerLat !== undefined &&
      job.customerLat !== null &&
      job.customerLng !== undefined &&
  ...
  function hasDriverGps(job) {
    return (
      job.driverLat !== undefined &&
      job.driverLat !== null &&
      job.driverLng !== undefined &&
      job.driverLng !== null
    );
  }
  ...
  const filteredBookings = bookings.filter((job) => {
    ...
    const searchText = `${job.id} ${job.name || ""} ${job.phone || ""} ${
      job.registration || ""
    } ${job.vehicle || ""} ${job.driverName || ""}`.toLowerCase();
        ...
        {filteredBookings.map((job) => (
          <div className="jobCard" key={job.id}>
            ...
            {hasCustomerGps(job) && (
              <div className="gpsBox">
                <div>
                  ...
                  <p>
                    {Number(job.customerLat).toFixed(6)},{" "}
                    {Number(job.customerLng).toFixed(6)}
                  </p>
                </div>
            ...
            {hasDriverGps(job) && (
              <div className="gpsBox driverGpsBox">
                <div>
                  ...
                  <p>
                    {Number(job.driverLat).toFixed(6)},{" "}
                    {Number(job.driverLng).toFixed(6)}
                  </p>
                </div>
            ...
            <div className="jobGrid">
              ...
              <div>
                <small>ETA Mode</small>
                <strong>{job.etaMode || "manual"}</strong>
              </div>

              <div>
                <small>Customer GPS</small>
                <strong>{hasCustomerGps(job) ? "Shared" : "Not shared"}</strong>
              </div>
            ...
            <div className="driverAssignment">
              ...
              <input
                ...
                onBlur={(e) =>
                  updateJob(job.id, { driverName: e.target.value })
                }
              />

              <input
                placeholder="Driver phone"
                defaultValue={job.driverPhone || ""}
                onBlur={(e) =>
                  updateJob(job.id, { driverPhone: e.target.value })
                }
              />

              <input
                placeholder="Driver vehicle"
                defaultValue={job.driverVehicle || ""}
                onBlur={(e) =>
                  updateJob(job.id, { driverVehicle: e.target.value })
                }
              />
            ...
            <div className="etaControls">
              <input
                ...
                value={job.manualEta || ""}
                onChange={(e) =>
                  updateJob(job.id, { manualEta: e.target.value })
                }
              />
            ...
            <div className="linkInputs">
              ...
              <input
                placeholder="Paste photo/proof URL"
                defaultValue={job.photoUrl || ""}
                onBlur={(e) => updateJob(job.id, { photoUrl: e.target.value })}
              />

              <input
                placeholder="Paste signature URL"
                defaultValue={job.signatureUrl || ""}
                onBlur={(e) =>
                  updateJob(job.id, { signatureUrl: e.target.value })
                }
              />
            </div>
          </div>
