    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        alert(
          "GPS error: " +
            error.message +
            ". Please allow location permission or use the manual map option."
        );
      },
      {
  ...
  if (!booking) {
    return (
      <main className="trackingPage">
        <section className="trackingCard">Loading...</section>
      </main>
    );
  }
    ...
    <main className="trackingPage">
      <section className="trackingCard">
        ...
        <div className="infoBox">
          <strong>Current ETA</strong>
          <span>{booking.eta || "Awaiting ETA"}</span>
        ...
        <div className="infoBox">
          ...
          <span>{booking.customerGpsShared ? "Shared" : "Not shared yet"}</span>
          {booking.customerLat && booking.customerLng && (
            <a href={mapLink(booking.customerLat, booking.customerLng)} target="_blank" rel="noreferrer">
              View My Shared Location
            </a>
          )}
        </div>
        ...
        {booking.driverLat && booking.driverLng && booking.driverTrackingActive && booking.status !== "Completed" && (
          <div className="driverLocationBox">
            <strong>Driver live location</strong>
            <span>{booking.driverLat}, {booking.driverLng}</span>
            <a href={mapLink(booking.driverLat, booking.driverLng)} target="_blank" rel="noreferrer">
        ...
        <div className="statusList">
          {[
            ...
            <div key={status} className={statusReached(status) ? "statusStep done" : "statusStep"}>
              <span />
              {status}
            </div>
          ))}
        ...
        <div className="infoBox">
          <strong>Notes</strong>
          <span>{booking.notes || "N/A"}</span>
        </div>

        <button className="shareBtn" onClick={shareMyLocation} disabled={saving}>
          {saving ? "Saving Location..." : "Share My Location"}
        </button>

        {manualHelp && (
          <div className="infoBox">
            <strong>Manual GPS Help</strong>
            <span>
              Open Google Maps, tap your blue dot, press Share location, then send it to McDowell on WhatsApp.
            </span>
        ...
        <a
          ...
          href={`https://wa.me/${BUSINESS_PHONE}?text=${encodeURIComponent(
            `Hi McDowell, I am checking job ${id}`
          )}`}
          target="_blank"
          rel="noreferrer"
        >
        ...
        <div className="infoBox">
          ...
          <span>{booking.paymentStatus || "Payment Pending"}</span>
        </div>

        {booking.invoiceUrl ? (
          <a className="invoiceBtn" href={booking.invoiceUrl} target="_blank" rel="noreferrer">
            View Invoice
          </a>
        ) : (
          <div className="invoiceBtn muted">No Invoice Yet</div>
        )}
      </section>
    </main>
  );
